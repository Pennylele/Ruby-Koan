...
class AboutClassMethods < Neo::Koan
  ...
  def test_objects_have_methods
    fido = Dog.new
    assert fido.methods.size > 0 #assert: Tests if test is true.
  end

  def test_classes_have_methods
    assert Dog.methods.size > 0 #It returns all the methods of class Dog, which inherits from object which gives it a quite large number of default methods.
  end
#http://stackoverflow.com/questions/35765353/what-does-the-assert-fido-methods-size-0-mean-in-the-ruby-code/35765575#35765575
 
  def test_you_can_define_methods_on_individual_objects
    fido = Dog.new
    def fido.wag
      :fidos_wag #This is a symbol - A Ruby symbol is a thing that has both a number (integer) representation and a string representation.
    end
...
#http://www.troubleshooters.com/codecorn/ruby/symbols.htm#_What_are_symbols
  ...
  def test_classes_and_instances_do_not_share_instance_variables
    ...
    assert_equal nil, Dog.name #why this is nil? 
  end
  ...
  class Dog
    class << self #another way to define a class
