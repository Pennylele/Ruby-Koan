...
class AboutMessagePassing < Neo::Koan
  ...
  def test_catching_messages_makes_respond_to_lie
    ...
    assert_equal false, catcher.respond_to?(:any_method) #It is false because the method name doesn't exist, under which circumstances, the catcher didn't respond to the method "any_method".
    #What catcher responds to is the default method "method_missing".
  end
  ...
  class WellBehavedFooCatcher
    def respond_to?(method_name)
      if method_name.to_s[0,3] == "foo"
        ...
        super(method_name) #not so sure what does the super(method_name) mean?
      end
    end
  end
