...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    assert_equal "a", "abbcccddddeeeee"[/az*/]
    assert_equal "", "abbcccddddeeeee"[/z*/] #?? Not sure why does the asterisk affect the asnwer??
  ...
  def test_slash_w_is_a_shortcut_for_a_word_character_class
    ...
    assert_equal "variable_1", "variable_1 = 42"[/\w+/] #Matches any word character (alphanumeric & underscore). 
  end
  ...
  def test_sub_is_like_find_and_replace
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] } #"one two-three".sub(/(t\w*)/) { $1[0, 1] } searches for the pattern t\w*. This finds the substring "two". Since the whole thing is in a capturing group, this substring is stored in $1. Now the block is called and returns "two"[0,1], which is "t". So "two" is replaced by "t" and you get "one t-three".
  end
